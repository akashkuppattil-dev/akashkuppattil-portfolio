        ...
        <div className="flex flex-col lg:flex-row gap-20 items-stretch">
          ...
          <div className="lg:w-1/2 w-full space-y-4 flex flex-col justify-center">
            {contactLinks.map((link, idx) => (
              ...
              >
                <Card className="p-8 glass-card border border-white/5 flex items-center justify-between group-hover:-translate-y-1 transition-all duration-500">
                  <div className="flex items-center gap-6">
                    <div className="w-14 h-14 rounded-[1.25rem] bg-accent/5 flex items-center justify-center text-accent group-hover:bg-accent group-hover:text-background transition-all duration-500">
                      <link.icon className="w-7 h-7" />
                    </div>
                    <div>
                      <p className="text-[10px] font-black text-accent/60 uppercase tracking-[0.2em] mb-1">{link.label}</p>
                      <p className="text-lg sm:text-2xl font-black tracking-tighter uppercase break-all leading-none">{link.value}</p>
                    </div>
                  </div>
                  <div className="w-12 h-12 rounded-full border border-white/5 flex items-center justify-center group-hover:bg-accent group-hover:border-accent group-hover:rotate-45 transition-all">
                    <ArrowUpRight className="w-5 h-5 text-accent group-hover:text-background" />
                  </div>
                </Card>
              </motion.a>
            ))}
